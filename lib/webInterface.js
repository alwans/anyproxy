...
const { saveRuleInfo, fetchRuleList } = require('./extends/data/dataHandle');
...
class webInterface extends events.EventEmitter {
  ...
  getServer() {
    ...
    //新增---wl
    app.post('/api/saveRuleInfo',async (req,res) => {
      res.setHeader('Access-Control-Allow-Origin', '*');
      let reqData = req.body;
      await saveRuleInfo(reqData);
      res.json({
        status: 'success',
        code: 'done'
      });
    ...
    app.get('/api/fetchProxyRuleList', async (req,res) =>{
      const query = req.query;
      if (query) {
          let proxyRuleList = await fetchRuleList(query);
          res.json({
            status: 'success',
