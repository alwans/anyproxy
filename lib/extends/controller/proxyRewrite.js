'use strict'
const {urlDecode,urlEncode} = require('./utils.js');

var isMathValue = false;

/**
 * 重写逻辑入口
 * @param {*} requestDetail 原始请求数据
 * @param {*} sql_result 接口修改信息
 */
 exports.rewrite_handler = function(requestDetail,sql_result,responseDetail){
    filter_sql_result(sql_result); // format sql_result
    if(sql_result.originHeaderParam !== '' || sql_result.targetHeaderParam !== ''){
        header_handler(requestDetail,sql_result,responseDetail);
    }
    if( sql_result.originBody !== '' || sql_result.targetBody !== '' || sql_result.httpStatusCode != ''){
        if(sql_result.isReqData===1){
            req_body_handler(requestDetail,sql_result);
        }else{
            res_body_handler(sql_result,responseDetail);
        }
    }
 }
 
 
 /**
  * 请求头逻辑入口
  * @param {*} requestDetail 原始请求数据
  * @param {*} sql_result 接口对应的信息
  * @param {*} responseDetail 响应数据
  */
 const header_handler = function(requestDetail,sql_result,responseDetail){
    let arr1 = JSON.parse(sql_result.originHeaderParam || '[]');
    let arr2 = JSON.parse(sql_result.targetHeaderParam || '[]');
    let type = sql_result.isReqData===0?0:1; //0:req data  1:res data
    arr1.map((obj) => {
        let r = isExist(obj,arr2);
        if(r.flag){
            let index = arr2.indexOf(r.obj)
            arr2.splice(index,1);
            if(type===0){
                header_modify(requestDetail,obj,r.obj,type);
            }else{
                header_modify(responseDetail,obj,r.obj,type);
            }
        }else{
            if(type===0){
                header_remove(requestDetail,obj,type);
            }else{
                header_remove(responseDetail,obj,type);
            }
        }
    });
    arr2.map((obj) => {
        if(type===0){
            header_add(requestDetail,obj,type);
        }else{
            header_add(responseDetail,obj,type);
        }
        
    });
 }

/**
 * 修改响应数据
 * @param {*} sql_result 修改相关信息
 * @param {*} responseDetail 原始响应数据
 */
 function res_body_handler(sql_result,responseDetail){
    // let str = urlDecode(responseDetail.response.body.toString('utf-8'),'utf-8');
    let str = responseDetail.response.body.toString('utf-8');
    // let r1 = new RegExp(v,'ig');
    // str = str.replace(/\s+/g, ""); //去掉请求数据中的空格
    let pre_arr = sql_result.originBody ===''? []: sql_result.originBody.replace(/\s+/g, "").split('&'); //去掉空格
    let target_arr = sql_result.targetBody===''? []: sql_result.targetBody.replace(/\s+/g, "").split('&');//去掉空格
    if(sql_result.httpStatusCode !== ''){
        responseDetail.response.statusCode = sql_result.httpStatusCode;
        // return responseDetail;
    }
    pre_arr.map((v,num) =>{
        if(sql_result.isGlobal===0 || sql_result.isGlobal === ''){
            let re = new RegExp(v,'i');
            // str = str.replace(re,target_arr[num]);
            str = str.replace(v,target_arr[num]);
        }else{
            let re = new RegExp(v,'ig');
            str = str.replace(re,target_arr[num]);
        }
    });
    responseDetail.response.body = Buffer.from(str,'utf-8');
    // console.log(responseDetail.response.body.toString('utf-8'));
 }

 /**
  * req body 逻辑入口
  * @param {*} requestDetail 
  * @param {*} sql_result 
  */
 const req_body_handler = function(requestDetail,sql_result){
    if(requestDetail.requestOptions.method==='GET'){
        req_get_body_handler(requestDetail,sql_result);
    }else{
        req_post_body_handler(requestDetail,sql_result);
    }
 }

 /**
  * post请求body修改
  * @param {*} requestDetail 
  * @param {*} sql_result 
  */
const req_post_body_handler = function(requestDetail,sql_result){
    let str = urlDecode(requestDetail.requestData.toString('utf-8'),'utf-8');
    // let str = requestDetail.requestData.toString('utf-8');
    // str = urlDecode(str,'utf-8');
    let pre_arr = sql_result.originBody===''? []: sql_result.originBody.split('&');
    let target_arr = sql_result.targetBody===''? [] : sql_result.targetBody.split('&');
    if(pre_arr.length!=target_arr.length) return requestDetail;
    pre_arr.map((v,num) =>{
        if(sql_result.isGlobal===0 || sql_result.isGlobal === ''){
            let re = new RegExp(v,'i');
            str = str.replace(re,target_arr[num]);
        }else{
            let re = new RegExp(v,'ig');
            str = str.replace(re,target_arr[num]);
        }
    });
    // str = urlEncode(str,'utf-8');
    requestDetail.requestData = Buffer.from(str,"utf-8");
    // console.log(urlDecode(requestDetail.requestData.toString('utf-8'),'utf-8'));
}

/**
 * get方法的body修改
 * @param {*} requestDetail 
 * @param {*} sql_result 
 */
 const req_get_body_handler = function(requestDetail,sql_result){
    let pre_arr = JSON.parse(sql_result.originBody || '[]');
    let target_arr = JSON.parse(sql_result.targetBody || '[]');
    let param_key=[];
    let param_value=[];
    requestDetail.url.split('?')[1].split('&').map((p) => {
        param_key.push(p.split('=')[0]);
        param_value.push(p.split('=')[1]);
    });
    pre_arr.map((obj) =>{
        let r = isExist(obj,target_arr);
        if(r.flag){
            param_value = get_body_modify(param_key,param_value,target_arr,obj,r);
        }else{
            param_value = get_body_remove(param_key,param_value,obj,r);
        }
    });
    target_arr.map((obj) => { // for add body params
        let filed = Object.keys(obj)[0];
        param_key.push(filed);
        param_value.push(obj[filed]);
    });
    let new_param_str = '';
    param_key.map((k,num) =>{
        new_param_str = new_param_str+k+'='+ param_value[num]+'&'
    });
    new_param_str = new_param_str.substring(0,new_param_str.length-1);
    let new_url = requestDetail.url.split('?')[0]+'?'+new_param_str;
    requestDetail.requestOptions.path = requestDetail.requestOptions.path.split('?')[0]+'?'+new_param_str;
    requestDetail.url = new_url;
    requestDetail._req.url = new_url;
 }

/**
 * 删除get的请求中指定的k=v
 * @param {*} param_key 
 * @param {*} param_value 
 * @param {*} pre_obj 
 * @param {*} r 
 */
 const get_body_remove = function(param_key,param_value,pre_obj,r){
    if(param_key.includes(r.filed)){
        let n1 = param_key.indexOf(r.filed);
        if(isMathValue && pre_obj[r.filed] !== param_value[n1]) return param_value; //math value is equal
        param_value.splice(n1,1);
    }else{
        console.log(`${filed} is not exist, this param remove failed ...!!!`);
    }
    return param_value;
 }

/**
 * 修改get请求中指定key对应的value
 * @param {*} param_key 
 * @param {*} param_value 
 * @param {*} target_arr 
 * @param {*} pre_obj 
 * @param {*} r 
 */
 const get_body_modify = function(param_key,param_value,target_arr,pre_obj,r){
    let index = target_arr.indexOf(r.obj);
    target_arr.splice(index,1);
    if(param_key.includes(r.filed)){
        let n1 = param_key.indexOf(r.filed);
        if(isMathValue && pre_obj[r.filed] !== param_value[n1]) return param_value; //math value is equal
        param_value[n1] = r.obj[r.filed];
    }else{
        console.log(`${filed} is not exist, this param modify failed ...!!!`);
    }
    return param_value;
 }

 /**
  * 新增请求头
  * @param {*} requestDetail 
  * @param {sql_obj} obj 
  */
 const header_add = function(requestDetail,obj,type){
    let filed = Object.keys(obj)[0];
    if(type===0){
        requestDetail.requestOptions.headers[filed] = obj.filed;
    }else{
        requestDetail.response.header[filed] = obj.filed;
    }
 }
 /**
  * 修改请求头
  * @param {*} requestDetail 
  * @param {*} pre_obj 请求头原始数据
  * @param {*} target_obj 请求头需要修改后的数据
  */
 const header_modify = function(requestDetail,pre_obj,target_obj,type){
    let filed = Object.keys(pre_obj)[0];
    if(type===0){
        requestDetail.requestOptions.headers[filed] = target_obj[filed];
    }else{
        requestDetail.response.header[filed] = target_obj[filed];
    }
 }
/**
 * 删除请求头
 * @param {*} requestDetail 
 * @param {*} obj 
 */
 const header_remove = function(requestDetail,obj,type){
    let filed = Object.keys(obj)[0];
    if(type===0){
        delete requestDetail.requestOptions.headers[filed]
    }else{
        requestDetail.response.header[filed];
    }
 }

/**
 * 对用的请求头中的key是否存在
 * @param {*} obj 
 * @param {*} objs 
 */
 const isExist = function(obj,objs){
    // objs.hasOwnProperty(obj);
    let filed = Object.keys(obj)[0];
    let flag = false;
    let target_obj = '';
    objs.some(v => {
        let f = Object.keys(v)[0];
        if(f===filed){
            flag = true;
            target_obj = v;
            return true;
        }
    });
    return {
        flag:flag,
        obj:target_obj,
        filed:filed
    };
}

/**
 * format sql_result , if v=null || '', then set v=''
 * @param {*} sql_result 
 */
const filter_sql_result = function(sql_result){
    for(let k in sql_result){
        sql_result[k] = sql_result[k] || '';
    }
}